...
class CommentsArticle extends React.Component {
    ...
    render() {
        ...
        const commentsLink = this.commentsExist() ? <a className="comments" href="#" onClick={this.showComments.bind(this)}>{commentsLinkText} comments ({this.props.comments.length})</a> : null;

        return (
            <div>
                {commentsLink}
                <ul className="comments" s>{commentsElement}</ul>
            </div>
        );
    }
